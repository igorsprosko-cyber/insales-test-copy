...
## 12. EVIDENCE-GATE — 2026-09-11

Обнаружено: (1) код, впоследствии описанный как «Patch #1», тестировался на InSales
до того, как соответствующий результат был задокументирован, без явной ссылки на commit
SHA в тексте отчёта на момент теста; (2) `v2.3-page-standardization-final2` была
force-push'нута без объявления, что стёрло из истории коммит `824ef73` (снятие
render-gate) и revert-коммит `17e299d`, которым его исправили. Итоговое содержимое файла
после force-push совпало с намеченным результатом, но сам факт скрытой перезаписи
истории нарушает evidence-протокол.

Правила на будущее:

- Любое изменение кода СНАЧАЛА коммитится в `codex-ready` (с показом diff и
  `git diff --check`), и только ПОСЛЕ этого вручную заливается на InSales для теста.
  Ничего не редактируется на живом сайте в обход git.
- Каждый PageSpeed/DevTools прогон сопровождается точным commit SHA, который был
  живым на сайте в момент теста.
- **Force-push в `v2.3-page-standardization-final2` или `codex-ready` запрещён без
  явного подтверждения пользователя.** Откат ошибочных коммитов делается только через
  `git revert` (новый коммит, видимая история), никогда через перезапись существующей
  истории веток.
